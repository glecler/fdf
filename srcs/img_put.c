...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/28 14:29:11 by glecler           #+#    #+#             */
/*   Updated: 2019/11/03 01:09:25 by glecler          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_put_pixel_img(t_data *data, int x, int y, int z)
{
	...
	while (i < 3)
	{
		*((data->addr) + i) = color / ft_pow(10, 3 * (b - 2));
		color = color - (color / ft_pow(10, 3 * (b - 2)) *
			ft_pow(10, 3 * (b - 2)));
		b--;
		i++;
	}
...
void	ft_put_line_img(t_coord a, t_coord b, t_data *data, int z)
{
	...
	if (b.x == a.x || ft_abs(coeff) > 1)
		ft_put_line_vert(a, b, data, z);
	while ((a.x + x < b.x) && ft_abs(coeff) <= 1)
	{
		y = ceil(x * coeff) + a.y;
		res = ft_put_pixel_img(data, ((a.x + x)) + data->posx,
			y + data->posy, z);
		x += data->step;
	}
}
